...
const Signin = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post("http://localhost:5112/api/Auth/login", { // Fixed URL to /api/Auth/login
        Email: email,
        Password: password,
      });
      ...
      if (response.status === 200) {
        ...
        // Temporarily skip token storage until backend returns a token
